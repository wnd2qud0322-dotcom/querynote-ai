...
function PDFTab({rl,notes,setNotes,setSql,book,pdfUrl,pi,setPi}){
  ...
  const doAI=async()=>{
    ...
    setAn(text);
    const updated={...notes,[key]:{raw:ni,ai:text,page:currentPage,title:cur.title,date:new Date().toLocaleDateString("ko-KR")}};
    setNotes(updated);
    ...
    <div style={{flex:1,display:"flex",gap:8,overflow:"hidden",padding:8}}>
      ...
      <div style={{width:310,background:C.white,border:`2px solid ${C.border}`,borderRadius:10,display:"flex",flexDirection:"column",overflowY:"auto",flexShrink:0,boxShadow:"0 4px 12px rgba(27,37,96,0.07)"}}>
        <div style={{padding:"18px 18px 14px",borderBottom:`1px solid ${C.soft}`,background:"linear-gradient(135deg,#EEF2FF,#FFFFFF)"}}>
          ...
          <button onClick={async()=>{
            // 시험 출제 가능성 표시 → 약점 후보 기록
            setNotes(prev=>{
              const examKey=`exam:${key}`;
              const n={...prev,[examKey]:{type:"exam",page:currentPage,title:cur.title,date:new Date().toLocaleDateString("ko-KR"),book:book||"unknown"}};
              try{window.storage.set("notes",JSON.stringify(n));}catch(e){}
              return n;
            });
          }} style={{marginTop:8,background:C.yL,border:`1px solid ${C.yellow}`,borderRadius:6,padding:"5px 12px",fontSize:12,cursor:"pointer",color:C.navy,fontWeight:700}}>
            ⭐ 시험 출제 가능성
          </button>
        </div>
...
function WeakTab({weak,notes,setNotes,book,onGoToPdf,onGoToSql,onGoToQuiz}){
  const wl=Object.values(weak);
  // notes 키가 "book:page" 형식이므로 현재 book 기준으로 필터링
  const bookPrefix=`${book||"default"}:`;
  const nl=Object.values(notes).filter(n=>n&&n.page&&!n.type);
  // 시험 출제 가능성 표시 목록
  const el=Object.values(notes).filter(n=>n&&n.type==="exam");
  const [sel,setSel]=useState(null);
        ...
        {sel?(
          sel._isNote?(
            ...
            <>
              ...
              <div style={{display:"flex",gap:10}}>
                <Btn primary style={{flex:1}} onClick={()=>onGoToPdf&&onGoToPdf(sel.page)}>📄 PDF로 이동</Btn>
                <Btn style={{flex:1}} onClick={()=>onGoToSql&&onGoToSql()}>🔬 SQL 실습</Btn>
                <Btn style={{flex:1,color:C.yellow,borderColor:C.yellow}} onClick={()=>onGoToQuiz&&onGoToQuiz()}>↺ 퀴즈 풀기</Btn>
              </div>
            </>
          )
